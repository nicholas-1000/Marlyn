...
namespace rbx
{
	...
	void scan_game()
	{
		...
		ReadProcessMemory(GetCurrentProcess(), (LPCVOID)(reinterpret_cast<std::uint32_t>(GetModuleHandleA(nullptr)) + 0x323B71C), &datamodel, sizeof(datamodel), 0);
		ReadProcessMemory(GetCurrentProcess(), (LPCVOID)(reinterpret_cast<std::uint32_t>(GetModuleHandleA(nullptr)) + 0x3222C7C), &visualengine, sizeof(visualengine), 0);
